...
bool isLeafInFrustum(const std::array<QVector4D, 4>& frustum, const qvec3f& mins, const qvec3f& maxs) {
    for (auto &plane : frustum) {
        // Select the p-vertex (positive vertex) - the vertex of the bounding
        // box most aligned with the plane normal
        qvec3f pVertex = qvec3f(
        ...
        if (plane.x() * pVertex[0] + plane.y() * pVertex[1] + plane.z() * pVertex[2] + plane.w() < 0) {
            return false;
        }
    }
...
void GLView::updateFaceVisibility(const std::array<QVector4D, 4>& frustum)
{
    ...
    if (!in_solid) {
        if (auto it = m_decompressedVis.find(desired.clusternum); it != m_decompressedVis.end()) {
            ...
            BSP_VisitAllLeafs(bsp, bsp.dmodels[0], [&](const mleaf_t &leaf) {
                if (Pvs_LeafVisible(&bsp, pvs, &leaf) && isLeafInFrustum(frustum, leaf.mins, leaf.maxs)) {
    ...
    // TODO: Come up with some way of checking if vis has changed
